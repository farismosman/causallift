...
from causallift.nodes import imodel_for_each


class IModelForEchTest(unittest.TestCase):
    def setUp(self):
        pass

    def test_should_initialize_object_with_treatment_argument(self):
        model = imodel_for_each.IModelForTreatedOrUntreated(treatment_val=0)
    ...
    def test_should_not_initialize_object_with_treatment_argument_is_invalid(self):
        with self.assertRaises(AssertionError):
            imodel_for_each.IModelForTreatedOrUntreated(treatment_val=3)
    ...
    def test_should_set_logger_level(self):
        ...
        self.assertEqual(20, imodel_for_each.log.level)

        Loggers().setup(verbose=3)
        self.assertEqual(10, imodel_for_each.log.level)

    @mock.patch('causallift.nodes.imodel_for_each.log', autospec=True)
